...
if [ $? -ne 0 ] ; then
    cancelrun "** 错误：容器创建失败，请翻译以上英文报错，百度尝试解决问题！"
fi
...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $CONFIG_PATH/auth.json)" != 0 ]; then
        log "8.开始青龙内部配置"
        docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://ghproxy.com/https://github.com/zhiguagua/VIP/blob/main/Scripts/sh/1custom.sh)"
    else
...
log "全面部署已完成！enjoy!!! 运行一遍ql extra即可"
