...
docker_install
warn "根据汉字提示进行操作 代码仅供学习参考，本人对此不负任何责任，使用代表同意本条款"
# 配置文件保存目录
...
CONFIG_PATH=$JD_PATH/ql1/config
DB_PATH=$JD_PATH/ql1/db
REPO_PATH=$JD_PATH/ql1/repo
RAW_PATH=$JD_PATH/ql1/raw
SCRIPT_PATH=$JD_PATH/ql1/scripts
LOG_PATH=$JD_PATH/ql1/log
JBOT_PATH=$JD_PATH/ql1/jbot
DEPS_PATH=$JD_PATH/ql1/deps
...
# 容器名称
input_container_name() {
    echo -n -e "\e[33m\n二、请输入要创建的 Docker 容器名称[默认为：qinglong1]->\e[0m"
    read container_name
...
# 配置已经创建完成，开始执行
log "1.开始创建配置文件目录"
PATH_LIST=($CONFIG_PATH $DB_PATH $REPO_PATH $RAW_PATH $SCRIPT_PATH $LOG_PATH $JBOT_PATH $DEPS_PATH)
...
log "开始安装 依赖"
        if [ "$INSTALL_dependency" = true ]; then
            docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://ghproxy.com/https://raw.githubusercontent.com/zhiguagua/ql/main/dependency.sh)"
        fi
